...
func NewMinio(ctx context.Context, lg *logger.Glog, cfg config.Config) (*Minio, error) {
	client, err := minio.New(cfg.Minio.Endpoint, &minio.Options{
		Creds:  credentials.NewStaticV4(cfg.Minio.User, cfg.Minio.Password, ""),
		Secure: false, // ToDo: why do we rely on http?
	})
	if err != nil {
	...
	if !exists {
		...
		lg.Infof(`Bucket "%s" created`, cfg.Minio.Bucket)
	}
